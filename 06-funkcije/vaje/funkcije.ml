...
let rec mapi f list = 
  let rec mapi_ind f list index =
    match list with
    | [] -> []
    | x :: xs -> (f x index) :: (mapi_ind f xs (index + 1))
  in
  mapi_ind f list 0
...
| x :: y :: xs -> f (f x y ) (fold_left_no_acc f xs)
| x :: xs -> failwith "list too short."  
...
let rec apply_sequence f x n =
  let rec apply_sequence_aux f x n acc =  
    if n <= 0 then reverse acc else apply_sequence_aux f (f x) (n-1) (x :: acc)
  in
